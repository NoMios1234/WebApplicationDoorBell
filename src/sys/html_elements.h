...
void file_upload(String label)
{
    page_content += F("</form>");
    page_content += F("<form id=\"uploadform\" enctype=\"multipart/form-data\" method=\"post\" action=\"/upload\">");
    page_content += F("<input id=\"fileupload\" name=\"inobinfile\" type=\"file\" />");
    page_content += F("<input type=\"submit\" value=\"Загрузить\" id=\"submit\" /></form>");
}
...
void listDir()
{
    ...
    page_content += F("<table class=\"table\"><thead><tr><th>NAME</th><th>SIZE</th><th>Action</th></tr></thead><tbody>");
    while (file)
    {
        if (String(file.name()).indexOf(".mp3") > 0)
        {
            page_content += F("<tr><th scope=\"row\">");
            page_content += file.name();
            page_content += F("</th><td>");
            page_content += file.size();

            page_content += F("</td><td>");
            page_content += F("<input id=\"button\" type=\"button\" class=\"btn btn-light\" onclick=\"sendCustom('/custom?play=");
            page_content += file.name();
            page_content += F("');\" value=\"Play\">");
            page_content += F("<input id=\"button\" type=\"button\" class=\"btn btn-dark\" onclick=\"sendCustom('/custom?del=");
            page_content += file.name();
            page_content += F("');\" value=\"Del\">");
...
void streamHandler()
{
    page_content += F("<h2>ESP32-CAM Last Photo</h2>");
    page_content += F("<p>It might take more than 5 seconds to capture a photo.</p>");
    page_content += F("<div><img src=\"saved-photo\" id=\"photo\" width=\"70%\"></div>");
    Serial.println("Stream page was builded succesfully!");
}
void buttonStream(int color){
    page_content += F("<div id=\"messegeResult\"></div><input type=\"button\" value=\"Stream\" onclick=\"sendCustom('/capture');\"  class=\"btn btn-");
    switch (color){
        case 1: page_content += F("primary"); break;
        case 2: page_content += F("secondary"); break;
        case 3: page_content += F("success"); break;
